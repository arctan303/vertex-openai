...
- **初衷与适用场景**：本项目旨在提供一种极其轻便、开箱即用的 OpenAI to Gemini 接口转换方案，无需配置复杂的中间件或数据库，特别适合个人开发者或小团队快速自建大模型 API 转发节点。
- **关于数据持久化**：所有的运行数据（包括用户 Token、日志、API Key 轮询使用状态）均采用内存缓存，并辅以每 5 秒的异步落盘策略，以此在极限压榨极低内存的同时保证 I/O 性能。 
- **安全性提示**：如需对外开放公网访问服务，强烈建议通过 `.env` 中的 `API_KEY` 设置全局访问密码或至少设置 `ADMIN_KEY` 防止后台被未授权访问。考虑到存储机制的特殊性，请勿将本服务投入对数据一致性容灾要求极度苛刻的高可用生产集群。
